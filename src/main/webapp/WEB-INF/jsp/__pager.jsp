...
<div class="paging">
	...
	<div class="aleft" style="display: inline-block; float: left;">
		<span>共${pagerItem.rowCount }行，</span> </span><span><select
			onchange="location.href=changeUrlArg(location.href,'${pagerItem.paramPageSize }', this.options[this.options.selectedIndex].value);">
	...
	<div class="aright" style="display: inline-block; float: right;">
		...
		<c:set value="${pagerItem.pageIndex == 1}" var="isFisrtPage"
			scope="request" />
		<c:set value="${pagerItem.pageIndex == pagerItem.pageCount}"
		...
		<span>&nbsp;&nbsp;&nbsp;&nbsp;跳到第 <select
			onchange="location.href=changeUrlArg(location.href,'${pagerItem.paramPageIndex }', this.options[this.options.selectedIndex].value);">

				<c:forEach var="i" begin="1" end="${pagerItem.pageCount }">

					<option value="${i }"${pagerItem.pageIndex==i?"selected":""}>${i }</option>
				</c:forEach>
