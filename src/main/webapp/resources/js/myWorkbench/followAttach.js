var followAttach = {
	...
	init: function () {
		//new 附件上传的uploader
		followAttach.uploader = new plupload.Uploader({
	        ...
	        url : ctx + "/myWorkbench/follow/uploadFile.jhtml",//上传文件路径
            ...
            init : {
				FilesAdded: function(up, files) {
					for (var i = 0; i < files.length; i++) {
						var file = files[i];
						followAttach.createAttach(up,file);
					}
					followAttach.uploader.start();
					return false;
				},
	...
	createAttach:function(up,file){
		...
		$('#att_del_'+file.id).click(function(){
			...
			up.removeFile(file);
			var url=ctx + "/myWorkbench/follow/deleteFile.jhtml";
		...
		$('#att_download_'+file.id).click(function(){
			var fileId = $(this).attr('fileId');
			var attachId = $("#uploader_file_"+fileId).attr("attachId");
			window.open(ctx + "/myWorkbench/follow/downloadFile.jhtml"+"?attachId="+attachId);
		});
		if(file.attachId){
			$("#waiting_"+file.id).hide();
			$("#att_del_"+file.id).show();
