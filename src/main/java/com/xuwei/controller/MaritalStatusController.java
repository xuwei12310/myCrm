...
/**
 * 
 * @description:基础字典-婚姻状况
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月29日下午3:35:34
 * @author：zjy @version：1.0
 */
@Controller
@RequestMapping("/dict/maritalStatus")
public class MaritalStatusController extends BaseController<Dict> {
	@Resource
	...
	public MaritalStatusController() {
		setResourceIdentity("dict:maritalStatus");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.maritalStatus);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.maritalStatus));
			m.setIsSys(0);
			m.setDictType(DictType.maritalStatus.toString());
	...
	public Object update(Model model, Dict m) {
		...
		if (!hasUpdatePermission()) {
			...
			if (!oldM.getName().equals(m.getName())) {
				...
				wrapper.eq("dict_type", DictType.maritalStatus);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:maritalStatus:up") && !hasPermission("dict:maritalStatus:down")) {
