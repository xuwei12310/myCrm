...
/**
 * 
 * @description: 基础字典-跟进阶段
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月29日下午3:34:12
 * @author：xw 
 * @version：1.0
 */
@Controller
@RequestMapping("/dict/followStage")
public class FollowStageController extends BaseController<Dict> {
	@Resource
	private IDictService dictService;
	@Resource
	private DictMapper dictMapper;

	public FollowStageController() {
		setResourceIdentity("dict:followStage");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.followStage);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.followStage));
			m.setIsSys(0);
			m.setDictType(DictType.followStage.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
           	 ...
           	 wrapper.eq("dict_type",DictType.followStage);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:followStage:up") && !hasPermission("dict:followStage:down")) {
	...
	/**
	 * 
	 * @description:获取跟进阶段下拉框
	 * @createTime 2017年9月1日 下午3:39:49
	 * @author xw
	 * @return
	 */
	@RequestMapping("followStageCombobox")
	@ResponseBody
	public Object followStageCombobox(){
