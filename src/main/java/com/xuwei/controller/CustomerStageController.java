...
/**
 * 
 * @description:基础字典-客户阶段
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月29日下午3:35:07
 * @author：zjy @version：1.0
 */
@Controller
@RequestMapping("/dict/customerStage")
public class CustomerStageController extends BaseController<Dict> {
	@Resource
	...
	public CustomerStageController() {
		setResourceIdentity("dict:customerStage");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.customerStage);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("阶段名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.customerStage));
			m.setIsSys(0);
			m.setDictType(DictType.customerStage.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
           	 ...
           	 wrapper.eq("dict_type",DictType.customerStage);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:customerStage:up") && !hasPermission("dict:customerStage:down")) {
