...
/**
 * 
 * @description: 基础字典-付款方式 
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年9月4日下午4:02:25
 * @author：zyd
 * @version：1.0
 */
@Controller
@RequestMapping("/dict/payType")
public class PayTypeController extends BaseController<Dict> {
	@Resource
	...
	public PayTypeController() {
		setResourceIdentity("dict:payType");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.payType);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.payType));
			m.setIsSys(0);
			m.setDictType(DictType.payType.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
				 ...
				 wrapper.eq("dict_type",DictType.payType);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:payType:up") && !hasPermission("dict:payType:down")) {
	...
	/**
	 * 
	 * @description:获取付款方式下拉框
	 * @createTime 2017年9月5日 下午2:01:15
	 * @author zyd
	 * @return
	 */
	@RequestMapping("payTypeCombobox")
	@ResponseBody
