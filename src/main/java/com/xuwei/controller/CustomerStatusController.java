...
/**
 * 
 * @description:基础字典-客户状态
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月29日下午3:35:34
 * @author：zjy @version：1.0
 */
@Controller
@RequestMapping("/dict/customerStatus")
public class CustomerStatusController extends BaseController<Dict> {
	@Resource
	...
	public CustomerStatusController() {
		setResourceIdentity("dict:customerStatus");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.customerStatus);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("状态名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.customerStatus));
			m.setIsSys(0);
			m.setDictType(DictType.customerStatus.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
           	 ...
           	 wrapper.eq("dict_type",DictType.customerStatus);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:customerStatus:up") && !hasPermission("dict:customerStatus:down")) {
	...
	/**
	 * 
	 * @description:获取客户状态下拉框
	 * @createTime 2017年9月1日 下午3:38:40
	 * @author xw
	 * @return
	 */
	@RequestMapping("customerStatusCombobox")
	@ResponseBody
	public Object customerStatusCombobox(){
