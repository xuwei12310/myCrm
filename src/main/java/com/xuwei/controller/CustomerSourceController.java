...
/**
 * 
 * @description: 基础字典-客户来源
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月29日下午3:34:12
 * @author：zjy @version：1.0
 */
@Controller
@RequestMapping("/dict/customerSource")
public class CustomerSourceController extends BaseController<Dict> {
	@Resource
	...
	public CustomerSourceController() {
		setResourceIdentity("dict:customerSource");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.customerSource);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("来源名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.customerSource));
			m.setIsSys(0);
			m.setDictType(DictType.customerSource.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
           	 ...
           	 wrapper.eq("dict_type",DictType.customerSource);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:customerSource:up") && !hasPermission("dict:customerSource:down")) {
