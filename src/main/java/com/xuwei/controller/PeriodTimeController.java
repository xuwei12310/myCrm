 ...
 */
@Controller
@RequestMapping("/dict/periodTime")
public class PeriodTimeController extends BaseController<Dict> {
	@Resource
	...
	public PeriodTimeController() {
		setResourceIdentity("dict:periodTime");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.periodTime);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.periodTime));
			m.setIsSys(0);
			m.setDictType(DictType.periodTime.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
				 ...
				 wrapper.eq("dict_type",DictType.periodTime);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:periodTime:up") && !hasPermission("dict:periodTime:down")) {
	 ...
	 * @return
	 */
	@RequestMapping("periodTimeCombobox")
	@ResponseBody
