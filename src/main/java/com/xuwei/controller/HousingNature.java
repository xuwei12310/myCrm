...
/**
 * 
 * @description: 基础字典-房屋性质
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月31日下午2:46:52
 * @author：zjy
 * @version：1.0
 */
@Controller
@RequestMapping("/dict/housingNature")
public class HousingNature extends BaseController<Dict> {
	@Resource
	private IDictService dictService;
	@Resource
	private DictMapper dictMapper;

	public HousingNature() {
		setResourceIdentity("dict:housingNature");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.housingNature);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.housingNature));
			m.setIsSys(0);
			m.setDictType(DictType.housingNature.toString());
	 ...
	 public Object update(Model model,Dict m){
        ...
        if(!hasUpdatePermission()){
           ...
           if(!oldM.getName().equals(m.getName())){
           	 ...
           	 wrapper.eq("dict_type",DictType.housingNature);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:housingNature:up") && !hasPermission("dict:housingNature:down")) {
