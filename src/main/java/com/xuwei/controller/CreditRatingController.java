...
/**
 * 
 * @description:基础字典-信用等级
 * @copyright: 福建骏华信息有限公司 (c)2017
 * @createTime: 2017年8月29日下午3:35:34
 * @author：zjy @version：1.0
 */
@Controller
@RequestMapping("/dict/creditRating")
public class CreditRatingController extends BaseController<Dict> {
	@Resource
	private IDictService dictService;
	@Resource
	private DictMapper dictMapper;

	public CreditRatingController() {
		setResourceIdentity("dict:creditRating");
	}
	...
	public Object create(Model model, Dict m) {
		...
		if (!hasCreatePermission()) {
			...
			ew.eq("dict_type", DictType.creditRating);
			if (dictService.selectCount(ew) > 0) {
				result.setMessage("名称已存在，请重新输入");
				return result;
			}
			m.setArray(dictService.nextArray(DictType.creditRating));
			m.setIsSys(0);
			m.setDictType(DictType.creditRating.toString());
	...
	public Object update(Model model, Dict m) {
		...
		if (!hasUpdatePermission()) {
			...
			if (!oldM.getName().equals(m.getName())) {
				...
				wrapper.eq("dict_type", DictType.creditRating);
	...
	public Object changeArray(Model model, @RequestParam(required = false) Long srcId,
		...
		ServiceResult result = new ServiceResult(false);
		if (!hasPermission("dict:creditRating:up") && !hasPermission("dict:creditRating:down")) {
